...
- If an upstream avatar overlay update introduces native composition staging, preserve those calls instead of simplifying back to the older window-bounds flow. In 26.608 the main bundle used `showWindowIfReady`, a staged opacity reveal, `setContentBounds`, and composition backing-canvas moves; the Linux patch still needs visibility recovery/frontmost behavior while preserving those native-composition side effects.
- For native-composition avatar drag drift, base the Linux drag repair on `getContentBounds()` and keep the `setWindowBounds(..., !1, !1)` call shape so backing canvases remain synchronized with the overlay window.
...
- The generated `node_repl` speaks MCP-style JSON lines. For direct installed-runtime probes, call the `js` tool through a `tools/call` JSON-RPC message; do not pipe raw JavaScript into stdin.
- If Chrome native host ping verification is needed, send a 4-byte length-prefixed JSON-RPC frame with `{"method":"ping"}`. The generated host does not answer legacy `{"type":"ping"}` probes.
- If Browser Use authenticated host fetch breaks after an upstream update, inspect the browser session registry instantiation in the main bundle. In 26.608 `appSessionId` used a symbol like `n.N`, not the older `*.t`; the matcher should accept general member exports while still injecting `appServerConnection`.
- If Linux remote-control availability breaks after an upstream update, inspect the feature-availability helper before patching. In 26.608 upstream added a Darwin `computerUseNodeRepl` guard before Windows/Linux branches; the Linux patch must add `control: true` without breaking that Darwin guard, Windows enablement, dev overrides, or `deviceAttestation`.
...
- In 26.608, upstream can use a retained Browser webview manager (`getRetainedWebview`) instead of the older explicit panel-host injection path. If `linuxBrowserViewportSurface` looks skipped because old panel-host anchors are absent, inspect the installed `thread-side-panel-tabs-*.js` and `browser-sidebar-manager-*.js` for retained-webview sync, `browser-sidebar-sync`, visible URL behavior, and host/root/capture-surface state before treating it as a regression.
...
- If Browser annotation text overlaps `Cancel` or the send button after typing a long note, keep the fix in Linux visual-compat CSS. The normal note editor surface should grow vertically for long notes, keep text wrapping/scrolling inside the textbox after a max height, and keep `Cancel`/`Save` bottom-aligned; still exclude `[data-browser-comment-design-prompt-shell]` and keep the `codexLinuxBrowserCommentComposerLayout` marker in `src/repack.js`.
...
- In 26.608, upstream can already ship the right-panel toolbar tab strip without the old outlet-order anchor. If `webview/assets/app-shell-*.js` contains `data-app-shell-tab-strip-controller`, `right-panel-tab-bar-header-spacer`, `RightPanelTabs`, `codex.rightPanel.expandFullWidth`, and the toolbar before/after-list anchors, classify the right-panel patch as `already-applied` instead of skipped.
...
For full release repacks, also verify the installed diagnostic manifest has no skipped patches:
